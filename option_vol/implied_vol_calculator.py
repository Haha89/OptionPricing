...
class ImpliedVolCalculator:
    ...
    def get_implied_vol_surface(self, underlying, path_png=None):
        ...
        options = self.get_listed_options(underlying)
        ...
        for o in options:
            o.set_implied_volatility(o.price)
            o.set_greeks()
        print("Generating plots")
        self.display_surfaces(options, 'implied_vol', path_png)

    @staticmethod
    def get_listed_options(underlying) -> List[BaseOption]:
        from option_vol.scrapping import Scrapping
        return Scrapping().parse_option(underlying=underlying)
